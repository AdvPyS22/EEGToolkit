...
class EEGData():
    ...
    def extract(self, \
        ...
        if p_event_type is None: return (self.extract(start_sec, stop_sec, event_type) for event_type in self.n_events.keys())
        if isinstance(p_event_type, (np.ndarray, list)): return (self.extract(start_sec, stop_sec, event_type) for event_type in p_event_type)
    ...
    def set_n_events(self) -> None:
        event_types = set([event[1] for event in self.events])
